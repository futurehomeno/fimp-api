...
| `device_lock`             | Device lock             | `int`      | `thermostat` or `power_regulator` service | Configure lock setting of the thermostat device.                                                                                                                                                                                                 |
| `used_sensors`            | Used sensor             | `int`      | `thermostat` service                      | Select which sensor to use. This will be use to calculate heater setting.                                                                                                                                                                        |
| `floor_sensor_resistance` | Floor sensor resistance | `int`      | `thermostat` service                      | Set the kΩ floor sensor rating, if is installed.                                                                                                                                                                                                 |